...
CREATE TABLE IF NOT EXISTS `mydb`.`program` (
  ...
  `user_id` VARCHAR(20) NOT NULL,
  `program_difficilty` VARCHAR(5) NOT NULL DEFAULT '초급자',
...
CREATE TABLE IF NOT EXISTS `mydb`.`article` (
  ...
  `article_property` TINYINT NULL,
  `article_video_id` VARCHAR(45) NULL DEFAULT 'null',
...
CREATE TABLE IF NOT EXISTS `mydb`.`like` (
  ...
  CONSTRAINT `fk_like_review1`
    ...
    REFERENCES `ssafit`.`review` (`review_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
