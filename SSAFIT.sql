...
CREATE TABLE IF NOT EXISTS `mydb`.`routine` (
  ...
  `routine_reg_date` TIMESTAMP NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS `mydb`.`program` (
  ...
  `program_reg_date` TIMESTAMP NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS `mydb`.`article` (
  ...
  `article_reg_date` TIMESTAMP NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS `mydb`.`review` (
  ...
  `review_reg_date` TIMESTAMP NOT NULL DEFAULT NOW(),
...
INSERT INTO user
VALUES ("beom", "1234", "김인범", "세상이 밉다", "ssafy@ssafy.com", "1997-02-10", 01093799366, 1, 200, 200, 200, 600, "몰루", "test"),
	("harvey", "1234", "박형석", "지구 망해라! 1일차", "ssafy@ssafy.com", "1997-01-20", 01012345678, 1, 100, 60, 120, 280, "몰루", "test2");

INSERT INTO article (user_id, article_title, article_content, article_property)
VALUES ("beom", "자유게시판", "테스트중", 1),
	("harvey", "루틴연구소", "화이팅!!", 0);

INSERT INTO user (user_id, user_password, user_name, user_nickname, email, birth, phone_no, gender, squat, bench_press, dead_lift, sbd, question_1, question_2) VALUES
('lee', '1234', '이순신', '임진왜란', 'lee@ssafy.com', '1997-03-10', 01093799367, 0, 110, 80, 60, 250, '아리랑', '임진록'),
('hong', '1234', '홍길동', '의적', 'hong@ssafy.com', '1997-04-10', 01093799368, 0, 120, 90, 70, 280, '토끼와 거북이', '장화홍련'),
('yoon', '1234', '윤동주', '서시', 'yoon@ssafy.com', '1997-05-10', 01093799369, 1, 130, 100, 80, 310, '그리운 고향', '별 헤는 밤'),
('kang', '1234', '강감찬', '고려의 명장', 'kang@ssafy.com', '1997-06-10', 01093799370, 0, 140, 110, 90, 340, '춘향전', '심청전'),
('park', '1234', '박지성', '축구 영웅', 'park@ssafy.com', '1997-07-10', 01093799371, 1, 150, 120, 100, 370, '태백산맥', '토지');

INSERT INTO `user` (`user_id`, `user_password`, `user_name`, `user_nickname`, `email`, `birth`, `phone_no`, `gender`, `squat`, `bench_press`, `dead_lift`, `sbd`, `question_1`, `question_2`) 
VALUES
('johnsmith', 'password1', 'John Smith', 'Jsmith', 'johnsmith@example.com', '1990-05-15', 1012345678, 1, 145, 180, 165, 490, 'Question 1', 'Question 2'),
('sarahjones', 'password2', 'Sarah Jones', 'SJones', 'sarahjones@example.com', '1985-08-20', 1023456789, 0, 120, 135, 155, 410, 'Question 1', 'Question 2'),
('mikebrown', 'password3', 'Mike Brown', 'MBrown', 'mikebrown@example.com', '1995-03-10', 1034567890, 1, 180, 160, 195, 535, 'Question 1', 'Question 2'),
('emilywilson', 'password4', 'Emily Wilson', 'EWilson', 'emilywilson@example.com', '1992-11-25', 1045678901, 0, 140, 120, 135, 395, 'Question 1', 'Question 2'),
('davidlee', 'password5', 'David Lee', 'Dlee', 'davidlee@example.com', '1988-07-01', 1056789012, 1, 190, 200, 185, 575, 'Question 1', 'Question 2'),
('jessicahall', 'password6', 'Jessica Hall', 'JHall', 'jessicahall@example.com', '1991-09-12', 1067890123, 0, 110, 140, 125, 375, 'Question 1', 'Question 2'),
('andrewclark', 'password7', 'Andrew Clark', 'AClark', 'andrewclark@example.com', '1993-04-05', 1078901234, 1, 155, 175, 150, 480, 'Question 1', 'Question 2');
...
VALUES("데드리프트", "하체", "데드리프트(deadlift)는 가장 대표적인 웨이트 트레이닝 운동 중 하나로, 바닥에 놓인 바벨을 잡고 팔을 구부리지 않은 자세로 엉덩이 높이까지 들어올리는 식으로 행한다. 복부 근육, 등 근육, 둔부 근육, 다리 근육 등 연관되는 근육이 광범위하여 널리 행해지며, 덤벨을 사용한다거나 한쪽 다리만을 사용하는 등 다른 운동효과를 위한 여러 가지 변형이 있다. 스쿼트, 벤치프레스와 함께, 높은 무게를 들어올리는 것으로 겨루는 스포츠 경기인 파워리프팅(Powerlifting)의 세 종목 중 하나이기도 하다. 벤치프레스와 합친 운동인 데드프레스도 있다.", "EBjYQeeBI-0&t=42s"),
	...
	("벤치프레스", "가슴", "벤치 프레스(bench press, chest press)는 스쿼트, 데드리프트와 함께 웨이트 트레이닝의 'Big 3'로 불리는 대표적인 근력 운동 중 하나이다. 벤치에 등을 대고 누워서 역기를 가슴 높이까지 내렸다가 굽힌 팔꿈치를 쭉 펴면서 들어 올리는 방식으로 실시한다.", "A2kHURY746E"),
    ...
    ("사이드레터럴레이즈", "어깨", "사이드 레터럴 레이즈는 어깨 측면의 디테일을 살려주고 볼륨감을 넣어주는 운동입니다.", "iNgwwI3WBTo"),
    ("오버헤드프레스", "어깨", "오버헤드 프레스는 역기 운동처럼 바벨을 번쩍 드는 운동입니다. 이 자세를 통해서 어깨를 자극하고 삼두근까지 근육을 키워줄 수 있습니다.", "7q4CKXhkIGE"),
    ("암풀다운", "등", "암(Arm) + 풀다운(Pull down)이라는 용어로 우리의 팔을 끌어 내리는 행위입니다.", "QoAYCFh57_o"),
    ("페이스풀", "어깨", "케이블을 활용한 후면 어깨를 타겟한 운동으로 얼굴 쪽으로 당기는 운동입니다. 어깨후면 삼각근과 등 상부를 발달시키며, 특히 회전근개 강화에 효과적인 동작입니다.", "v=2ZGmW0k-eUk"),
    ("업라이트로우", "어깨", "업라이트 로우는 승모근과 삼각근을 주로 사용하는 운동이다. 덤벨, 케이블, 스미스 머신을 사용할 수 있으며 동작에 변형을 줘서 주로 자극되는 근육 부위를 다르게 할 수 있다.", "sRWFEY1M_Jo"),
    ("벤트오버레터럴레이즈", "어깨", "벤트 오버 레터럴 레이즈는 어깨 후면을 발달시킬 수 있는 운동이다.", "n3aX0oJyvWA"),
    ...
    ("카프레이즈", "하체", "카프 레이즈는 종아리 근육을 강화시키는데 사용되는 운동으로, 발뒤꿈치를 들어올리는 운동이다", "QdAtQJMKi0U"),
    ("바벨컬", "팔", "이두 근육을 주로 타겟으로 하는 기본적인 운동 중 하나입니다. 이 운동은 바벨과 양손 그립을 사용하여 이두를 강화하고 발달시키는 데 효과적입니다.", "Dlg0W_5mq98"),
    ("덤벨컬", "팔", "이두 근육을 강화하는 데 효과적인 운동입니다. 이 운동은 덤벨을 사용하여 각 손을 독립적으로 움직여 이두를 타겟으로 합니다.", "y3aMzyKqvNM"),
    ("해머컬", "팔", "이두 근육과 전완 근육을 동시에 강화하는 운동입니다. 이두 근육의 두께와 팔의 강력한 힘을 개발하는 데 효과적입니다.", "n4L2Kvdtg-8"),
    ("덤벨프레스", "가슴", "가슴과 어깨 근육을 강화하는 운동입니다. 이 운동은 양손에 덤벨을 들고 가슴 근처에서 팔을 펼치는 동작으로 이루어집니다.", "DNNcr4_IskA"),
    ("바벨로우", "등", "허리를 곧게 펴고 손바닥을 내려다보게 하여 바벨을 잡습니다. 상체를 앞으로 기울이면서 바벨을 허리 쪽으로 당기고 내리면서 등을 수축시킵니다.", "j7Xtlx2S2g"),
    ("덤벨로우", "등", "좌우 손에 각각 덤벨을 들고 허리를 곧게 펴고 앞으로 기울입니다. 한 손씩 번갈아가며 덤벨을 허리 쪽으로 당기고 등을 수축시킵니다.", "VzpvMaGEiAA"),
    ("시티드케이블로우", "등", "시티드 케이블 로우 기계에 앉아 양 손으로 그립을 잡고 등을 수축시키며 손을 가슴 쪽으로 당깁니다.", "ZArSjRpaTV4"),
    ("원암덤벨로우", "등", "한 손에 덤벨을 들고 반대쪽 발을 앞으로 내딛고 허리를 곧게 펴고 앞으로 기울입니다. 한 손씩 번갈아가며 덤벨을 허리 쪽으로 당기고 등을 수축시킵니다.", "2i1v1UvsP_w"),
    ("덤벨플라이", "가슴", "등받이가 있는 벤치에 누워 양 손에 덤벨을 들고 팔을 측면으로 벌리면서 가슴 근육을 수축시키는 운동입니다.", "gmVan01OP9E"),
    ("케이블크로스오버", "가슴", "케이블 크로스오버 기계를 사용하여 가슴 근육을 타겟으로 하는 운동입니다. 팔을 측면으로 벌리면서 가슴을 가로지르는 동작을 수행합니다.", "zU7KivWmxII"),
    ...
    ("러시안트위스트", "복근", "등을 바닥에 대고 상체를 약간 뒤로 기울인 상태에서 양 손에 덤벨이나 메디슨볼을 들고 좌우로 허리를 돌리는 운동입니다. 측면 복근을 강화시키고 허리 유연성을 향상시킵니다.", "DMxf2aHLNHA"),
    ("덤벨숄더프레스", "어깨", "등받이가 있는 의자에 앉아 양 손에 덤벨을 들어올려 어깨 높이에서 시작하여 팔을 펴서 덤벨을 위로 밀어 올리는 운동입니다. 각 손을 독립적으로 움직여 어깨 근육을 균등하게 발달시킵니다.", "fx8mgLt7FR4"),
    ("프론트레이즈", "어깨", "서서 양 손에 덤벨을 들고 팔을 펴서 앞으로 들어올리는 운동입니다. 전면 어깨 근육을 타겟으로 합니다.", "u3zQI-PWaOw");